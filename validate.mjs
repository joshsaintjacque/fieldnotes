...
for (const [pattern, label] of [
  ...
  [/function setTodoistTaskSnapshot\(tasks, \{ savedAt, authEpoch, persisted, confirmed \}\) \{[\s\S]*?todoistTasks = tasks;[\s\S]*?todoistTaskSnapshotSavedAt = savedAt;[\s\S]*?todoistTaskSnapshotAuthEpoch = authEpoch;[\s\S]*?todoistHasTaskSnapshot = true;[\s\S]*?todoistTaskSnapshotPersisted = persisted;[\s\S]*?todoistTasksConfirmed = confirmed;/, 'Todoist snapshot state assignments are centralized'],
  [/const cacheResult = await writeTodoistTaskCache\(tasks, authEpoch, request\);[\s\S]*?if \(!cacheResult \|\| !todoistTaskLoadIsCurrent\(request, authEpoch\)\) return;[\s\S]*?const \{ cache, persisted \} = cacheResult;[\s\S]*?setTodoistTaskSnapshot\(tasks, \{ savedAt: cache\.savedAt, authEpoch, persisted, confirmed: true \}\);[\s\S]*?setTodoistStatus\(`Updated just now · \$\{visible\.length\} task/, 'live Todoist tasks render and confirm even when their cache was not persisted'],
  ...
  [/async function adoptTodoistTaskCache\(authEpoch, request\) \{[\s\S]*?const cache = await readTodoistTaskCache\(authEpoch\);[\s\S]*?if \(!todoistTaskLoadIsCurrent\(request, authEpoch\) \|\| await todoistAuthGet\(TODOIST_AUTH_EPOCH_KEY, null\) !== authEpoch\) return 'stale';[\s\S]*?if \(!cache\) return 'no-cache';[\s\S]*?setTodoistTaskSnapshot\(cache\.tasks, \{ savedAt: cache\.savedAt, authEpoch, persisted: true, confirmed: false \}\);[\s\S]*?const rendered = await renderTodoistTasks\(\{ preserveStatus: true, isCurrent: renderIsCurrent \}\);[\s\S]*?if \(!rendered \|\| !await renderIsCurrent\(\)\) return 'stale';[\s\S]*?setTodoistStatus\(`Showing saved tasks from \$\{formatTodoistCacheTime\(cache\.savedAt\)\} · Updating…`, false, true\);[\s\S]*?return 'adopted';/, 'cache adoption has guarded stale, no-cache, and adopted outcomes'],
  ...
  [/const request = todoistTaskRequest;\s*const cacheAdoption = await adoptTodoistTaskCache\(authEpoch, request\);\s*if \(cacheAdoption === 'stale'\) return;\s*void loadTodoistTasks\(\{ retainCurrentTasks: cacheAdoption === 'adopted' \}\);/, 'auth storage adoption handles stale, absent, and saved caches without duplicate state commits'],
  [/const request = \+\+todoistTaskRequest;\s*todoistActiveAuthEpoch = authEpoch;\s*const cacheAdoption = await adoptTodoistTaskCache\(authEpoch, request\);\s*if \(cacheAdoption === 'adopted'\) void loadTodoistTasks\(\{ retainCurrentTasks: true \}\);\s*else if \(cacheAdoption === 'no-cache'\) void loadTodoistTasks\(\);/, 'startup adoption handles stale, absent, and saved caches without duplicate state commits'],
