...
for (const [pattern, label] of [
  ...
  [/const l=await get\(LOCATION_KEY,null\)/, 'location restoration'],
...
for (const [pattern, label] of [
  [/todoistAuthStore = chrome\.storage\.local/, 'persistent Todoist auth storage'], [/TODOIST_AUTH_EPOCH_KEY = 'fieldTodoistAuthEpoch'/, 'persistent Todoist auth invalidation epoch'], [/setAccessLevel\(\{ accessLevel: 'TRUSTED_CONTEXTS' \}\)/, 'trusted-context local storage restriction'], [/chrome\.identity\.getRedirectURL\('oauth2'\)/, 'Chrome OAuth redirect'], [/code_challenge_method: 'S256'/, 'OAuth PKCE'], [/https:\/\/api\.todoist\.com\/oauth\/register/, 'dynamic public client registration'], [/https:\/\/api\.todoist\.com\/oauth\/access_token/, 'Todoist token exchange'], [/\/tasks\/filter\?\$\{params\}/, 'today task filter'], [/query: 'today', limit: '200'/, 'today filter and bounded page size'], [/while \(cursor\)/, 'Todoist cursor pagination'], [/JSON\.stringify\(\{ due_date: dueDate \}\)/, 'tomorrow mutation allowlist'], [/TODOIST_SNOOZE_MS = 60 \* 60 \* 1000/, 'one-hour local snooze'], [/TODOIST_AUTH_LOCK = 'field-notes-todoist-auth'/, 'cross-tab token refresh lock'], [/withTodoistAuthLock\(\(\) => invalidateTodoistAuthLocked\(/, 'disconnect serializes auth removal'], [/await todoistAuthSet\(TODOIST_AUTH_EPOCH_KEY, epoch\)/, 'OAuth records a persisted auth attempt'], [/TODOIST_AUTH_EPOCH_KEY, null\) !== authEpoch/, 'OAuth completion rejects invalidated attempts'], [/if \(error\.todoistCredentialsInvalid\) await invalidateTodoistAuthLocked\(latest\)/, 'definitively rejected refresh removes credentials'], [/if \(!isTodoistSession\(session\)\)/, 'malformed Todoist credentials are rejected'], [/TODOIST_SNOOZE_LOCK = 'field-notes-todoist-snooze'/, 'cross-tab snooze lock'], [/latest\.accessToken !== session\?\.accessToken/, 'rotated token preservation'], [/resource_types: '\["user"\]'/, 'Todoist account timezone lookup'], [/payload\?\.user\?\.tz_info\?\.timezone/, 'Todoist account timezone use'], [/area === 'local' && changes\[TODOIST_SNOOZE_KEY\]/, 'cross-tab snooze refresh'], [/area === 'local' && changes\[TODOIST_AUTH_KEY\]/, 'cross-tab Todoist auth change'], [/else if \(!todoistConnected \|\| !sameTodoistAccessToken\(changes\[TODOIST_AUTH_KEY\]\.oldValue, session\)\) \{[\s\S]*?void loadTodoistTasks\(\);/, 'cross-tab Todoist auth replacement reloads tasks'], [/const todoistApiPriority = task => \{[\s\S]*?priority >= 1 && priority <= 4 \? priority : 1;/, 'invalid Todoist priorities fall back to API P4'], [/visible\.filter\(task => todoistApiPriority\(task\) === priority\)/, 'Todoist groups use normalized API priorities'], [/const groups = \[\s*\[4, 'P1 · Must do'\],\s*\[3, 'P2 · Should do'\],\s*\[2, 'P3 · Do when you have time'\],\s*\[1, 'P4 · Optional'\]\s*\];/, 'Todoist API priority mapping and user-facing order'], [/heading\.textContent = `\$\{label\} · \$\{tasks\.length\} task\$\{tasks\.length === 1 \? '' : 's'\}`;/, 'Todoist priority label count grammar']
...
for (const token of ['chrome.storage.local', 'Todoist', 'Open-Meteo', 'topSites', 'Reset location', 'Disconnect Todoist', 'Clear local data', 'Limited Use', 'analytics']) if (!privacy.includes(token)) throw Error(`Privacy policy missing: ${token}`);
for (const token of ['dependency-free', 'Reset location', 'PRIVACY.md', 'THIRD_PARTY_NOTICES.md', 'MIT License']) if (!readme.includes(token)) throw Error(`README missing: ${token}`);
...
console.log(`Validated ${files.length} release files: MV3 permissions, no removed-service residue, location save/restore/reset, weather attribution, Todoist behavior, documentation, and license.`);
