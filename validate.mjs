...
for (const [pattern, label] of [
  ...
  [/const cache = await writeTodoistTaskCache\(tasks, authEpoch, request\);[\s\S]*?todoistTasks = tasks;[\s\S]*?setTodoistStatus\(`Updated just now · \$\{visible\.length\} task/, 'successful Todoist refresh overwrites the cache and shows freshness'],
  [/if \(retainCurrentTasks && todoistHasTaskSnapshot && todoistTaskSnapshotAuthEpoch === authEpoch\) \{[\s\S]*?todoistTasksConfirmed = false;[\s\S]*?setTodoistStatus\(`Showing saved tasks from \$\{formatTodoistCacheTime\(todoistTaskSnapshotSavedAt\)\} · Updating…`, false, true\);[\s\S]*?renderTodoistTasks\(\{ preserveStatus: true, isCurrent: renderIsCurrent \}\)/, 'refresh makes retained tasks read-only while keeping a visible updating status'],
  ...
  [/function clearTodoistTaskSnapshot\(\) \{[\s\S]*?todoistHasTaskSnapshot = false;[\s\S]*?todoistTasksConfirmed = false;/, 'Todoist invalidation and reset clear task confirmation'],
  ...
  [/todoistTasks = tasks;[\s\S]*?todoistHasTaskSnapshot = true;[\s\S]*?todoistTasksConfirmed = true;/, 'current live Todoist load confirms task actions'],
  ...
  [/async function renderTodoistTasks\(\{ preserveStatus = false, isCurrent = \(\) => true \} = \{\}\) \{[\s\S]*?const \{ visible, snoozed \} = await visibleTodoistTasks\(\);[\s\S]*?if \(!await isCurrent\(\)\) return null;[\s\S]*?container\.replaceChildren\(\);/, 'Todoist task DOM commits only after its current-request guard'],
  [/const renderIsCurrent = async \(\) => todoistTaskLoadIsCurrent\(request, authEpoch\) && await todoistAuthGet\(TODOIST_AUTH_EPOCH_KEY, null\) === authEpoch;[\s\S]*?const rendered = await renderTodoistTasks\(\{ preserveStatus: true, isCurrent: renderIsCurrent \}\);[\s\S]*?if \(!rendered \|\| !await renderIsCurrent\(\)\) return;[\s\S]*?setTodoistStatus\(`Updated just now/, 'Todoist success status rechecks request and stored epoch after rendering'],
  ...
  [/if \(todoistHasTaskSnapshot && todoistTaskSnapshotAuthEpoch === authEpoch\) \{[\s\S]*?setTodoistStatus\(`Showing saved tasks\$\{savedAt\} · Update failed\.`\);[\s\S]*?return;/, 'failed Todoist refresh retains only this account’s saved tasks'],
