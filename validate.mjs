...
for (const [pattern, label] of [
  ...
  [/if \(retainCurrentTasks && todoistHasTaskSnapshot && todoistTaskSnapshotAuthEpoch === authEpoch\) setTodoistStatus\(`Showing saved tasks from \$\{formatTodoistCacheTime\(todoistTaskSnapshotSavedAt\)\} · Updating…`, false, true\)/, 'manual refresh keeps a visible updating status'],
